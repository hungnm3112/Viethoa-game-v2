...
#### Bước 0.7.3: Quét lại dữ liệu vị trí & Thu thập Metadata (Backend)
- Viết script `tools/db/scan-occurrences.js` áp dụng thuật toán Tối ưu hóa (File-Centric) và Bulk Write Batches.
- Quét toàn bộ file gốc (`.xml`, `.btxt`) và cập nhật vị trí file vào MongoDB.
- **Thu thập Metadata bổ sung:**
  1. **Line Number:** Dòng code chứa text để tiện debug.
  2. **Format Variables:** Cờ `hasVariables` nếu chứa biến (`%1$s`, `\n`) để UI cảnh báo người dịch.
  3. **Context Node:** Ngữ cảnh thẻ XML (`<Item>`, `<Subtitle>`) giúp xác định nghĩa chuẩn xác.
  4. **File Extension:** Lưu riêng `.xml` hoặc `.btxt` để áp dụng luật Offset và luật Null Padding.
